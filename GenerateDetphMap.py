...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def load(self, image_path, depth_map, mask):
        ...
        depth_map = np.log(depth_map+0.00001, where=mask)

        depth_map = np.ma.masked_where(~mask, depth_map)

        depth_map = np.clip(depth_map, 0.1, np.log(max_depth+0.00001))
...
class DepthEstimationModel(tf.keras.Model):
    ...
    def train_step(self, batch_data):
        # batch_data return DataGenerator.__getitem__
        # 1. image => input 2. depth map => target
...
#     data=df.reset_index(drop="true"), batch_size=BATCH_SIZE, dim=(HEIGHT, WIDTH)
# )
# validation_loader = DataGenerator(
#     data=df2.reset_index(drop="true"), batch_size=BATCH_SIZE, dim=(HEIGHT, WIDTH)
# )
# model.fit(
#     train_loader,
#     epochs=EPOCHS,
...
#     iter(
#         DataGenerator(
#             data=df2[265:].reset_index(drop="true"), batch_size=6, dim=(HEIGHT, WIDTH)
#         )
#     )
# )
...
#     iter(
#         DataGenerator(
#             data=df2[300:].reset_index(drop="true"), batch_size=6, dim=(HEIGHT, WIDTH)
#         )
#     )
# )
# visualize_depth_map(test_loader, test=True, model=model)
